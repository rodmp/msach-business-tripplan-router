...
public class TripplanRouterConsumeTest {

  /**
   * 
   */
  @InjectMocks
  private TripplanRouterConsume tripplanRouterConsume;
  ...
  @Test
  public void getTripplanResources() throws Exception {
    String url = "https://apidev.meep.me";
    String uri = "/tripplan/api/v1";
    ...
    List<Resource> resourcesOld = mapper.readValue(objects, new TypeReference<List<Resource>>() {});

    when(feignConstants.getTripplanUrl()).thenReturn(url);
    when(feignConstants.getTripplanUri()).thenReturn(uri);

    @SuppressWarnings({"unchecked", "rawtypes"})
    ResponseEntity<List<Resource>> responseApi =
        new ResponseEntity(resourcesOld, HttpStatus.ACCEPTED);

    when(feignConstants.getLowerLeftLatLon()).thenReturn("38.711046,-9.160096");
    when(feignConstants.getUpperRightLatLon()).thenReturn("38.739429,-9.137115");
    when(feignConstants.getCompanyZoneIds()).thenReturn("545,467,473");

    when(restTemplate.exchange(Mockito.anyString(), Mockito.eq(HttpMethod.GET), Mockito.isNull(),
        Mockito.<ParameterizedTypeReference<List<Resource>>>any(), Mockito.anyString(), Mockito.anyString(),
        Mockito.anyString())).thenReturn(responseApi);

    assertNotNull(tripplanRouterConsume.getTripplanResources());
  }
}
